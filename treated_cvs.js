class HistoryManager {
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-4 py-3">
                    <div class="flex items-center space-x-3">
                        <span class="flex items-center justify-center w-8 h-8 rounded-full ${
                            analysis.results.globalMetrics.averageScore >= 80 ? 'bg-green-100 text-green-700' :
                            analysis.results.globalMetrics.averageScore >= 60 ? 'bg-blue-100 text-blue-700' :
                            'bg-red-100 text-red-700'
                        }">
                            ${index + 1}
                        </span>
                        <div>
                            <p class="font-medium text-gray-800">${analysis.jobData.title}</p>
                            <p class="text-sm text-gray-500">${analysis.results.candidates.length} CVs analysés</p>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3">
                    <span class="px-3 py-1 rounded-full text-sm ${
                        analysis.results.globalMetrics.averageScore >= 80 ? 'bg-green-100 text-green-800' :
                        analysis.results.globalMetrics.averageScore >= 60 ? 'bg-blue-100 text-blue-800' :
                        'bg-red-100 text-red-800'
                    }">
                        ${analysis.results.globalMetrics.averageScore.toFixed(1)}%
                    </span>
                </td>
                <td class="px-4 py-3">
                    <span class="px-3 py-1 rounded-full text-sm ${
                        ...
                        ${
                            analysis.results.globalMetrics.averageScore >= 80 ? 'Excellent' :
                            analysis.results.globalMetrics.averageScore >= 60 ? 'Bon' :
                            'À améliorer'
                        }
                    </span>
                </td>
                <td class="px-4 py-3">
                    <div class="flex flex-col">
                        <span class="font-medium text-gray-800">${analysis.jobData.title}</span>
                        <span class="text-sm text-gray-500">${analysis.jobData.department}</span>
                    </div>
                </td>
                <td class="px-4 py-3">
                    <div class="flex flex-col">
                        <span class="font-medium text-gray-800">${new Date(analysis.timestamp).toLocaleDateString('fr-FR')}</span>
                        <span class="text-sm text-gray-500">${new Date(analysis.timestamp).toLocaleTimeString('fr-FR')}</span>
                    </div>
                </td>
                <td class="px-4 py-3">
    ...
    viewDetails(analysisId) {
        const analysis = window.analysisStorage.findAnalysis(analysisId);
        if (!analysis) return;
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <div class="space-y-2">
                                <p><span class="font-medium">Poste :</span> ${analysis.jobData.title}</p>
                                <p><span class="font-medium">Département :</span> ${analysis.jobData.department}</p>
                                <p><span class="font-medium">Description :</span> ${analysis.jobData.description}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="bg-green-50 p-4 rounded-lg text-center">
                                ...
                                <p class="text-2xl font-bold text-green-700">${analysis.results.globalMetrics.averageScore.toFixed(1)}%</p>
                            </div>
                            <div class="bg-blue-50 p-4 rounded-lg text-center">
                                <p class="text-sm text-blue-600">Meilleur score</p>
                                <p class="text-2xl font-bold text-blue-700">${analysis.results.globalMetrics.bestScore.toFixed(1)}%</p>
                            </div>
                            <div class="bg-purple-50 p-4 rounded-lg text-center">
                                <p class="text-sm text-purple-600">CVs analysés</p>
                                <p class="text-2xl font-bold text-purple-700">${analysis.results.candidates.length}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-lg border border-gray-200">
                            ...
                            <div class="divide-y">
                                ${analysis.results.candidates.map((candidate, index) => `
    ...
    downloadReport(analysisId) {
        ...
        const reportContent = `
Rapport d'analyse - ${analysis.jobData.title}
Date: ${new Date(analysis.timestamp).toLocaleString('fr-FR')}

INFORMATIONS SUR L'OFFRE
-----------------------
Poste: ${analysis.jobData.title}
Département: ${analysis.jobData.department}
Description: ${analysis.jobData.description}

MÉTRIQUES GLOBALES
-----------------
Score moyen: ${analysis.results.globalMetrics.averageScore.toFixed(1)}%
Meilleur score: ${analysis.results.globalMetrics.bestScore.toFixed(1)}%
Nombre de CVs analysés: ${analysis.results.candidates.length}
...
`;
        ...
        a.href = url;
        a.download = `rapport_analyse_${analysis.jobData.title.toLowerCase().replace(/\s+/g, '_')}_${new Date(analysis.timestamp).toISOString().split('T')[0]}.txt`;
        ...
        if (index !== -1) {
            ...
            notification.innerHTML = `
                <div class="flex items-center space-x-2">
                    ...
                    <span>Analyse supprimée avec succès</span>
                </div>
            `;
